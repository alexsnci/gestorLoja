...
begin
    Result := script.sql('select i.*, e.descricao as produto, m.descricao as marca, e.cor, e.tamanho, e.modelo from itens_venda as i join estoque as e on i.idproduto = e.id join marcas as m on e.idmarca = m.id  where i.idvenda = :id')
                    .parametro('id',idvenda)
                    .abrir
                    .retornoJson;
end;
