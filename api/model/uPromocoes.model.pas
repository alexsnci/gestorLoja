...
interface
uses
  uInterface.promocoes, uScrips, uInterface.Scrips, Dataset.Serialize, System.JSON, Firedac.Comp.Client,System.SysUtils;
...
begin
    if produto <> '' then
    begin
        result := script.sql('select pr.id, pr.idestoque,pr.porcentagem,pr.valor,pr.valordesconto,pr.valoratual,pr.status, '+
        'p.descricao as produto, DATE_FORMAT(pr.datainicio,'+quotedStr('%d/%m/%Y')+') as datainicio, '+
        'DATE_FORMAT(pr.datafim,'+quotedStr('%d/%m/%Y')+') as datafim, '+
    ...
    begin
        ...
        'p.modelo, p.cor,s.descricao as setor,p.tamanho from promocoes '+
        'as pr join estoque as e on pr.idestoque = e.id join produtos as '+
        'p on e.idproduto = p.id join setores as s on p.idsetor = s.id').abrir.retornoJson;
    end;
end;
